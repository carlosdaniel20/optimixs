...
public function saveAutoBackupSettings(Request $request, Response $response, mysqli $db): Response
{
    // CSRF validation
    $data = (array) $request->getParsedBody();
    $csrfToken = $data['csrf_token'] ?? '';
    if (!\App\Support\Csrf::validate($csrfToken)) {
        return $this->jsonResponse($response, ['error' => __('CSRF inválido')], 403);
    }

    $frequency = $data['frequency'] ?? 'disabled';
    $overwrite = (int) ($data['overwrite'] ?? 0);

    $allowed = ['disabled', 'daily', 'weekly', 'monthly'];
    if (!in_array($frequency, $allowed)) $frequency = 'disabled';

    $configDir = dirname(__DIR__, 2) . '/storage/backups/';
    if (!is_dir($configDir)) mkdir($configDir, 0755, true);
    $configFile = $configDir . 'auto_backup_config.json';
    $config = [
        'frequency' => $frequency,
        'overwrite' => $overwrite,
        'last_run' => null,
    ...
    return $this->jsonResponse($response, ['success' => true, 'message' => __('Configuración guardada')]);
}
