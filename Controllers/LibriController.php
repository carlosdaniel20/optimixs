...
// --- GENERAR QR (NO BLOQUEANTE) ---
try {
    QRHelper::generateBookQR($id, $fields);
} catch (Throwable $e) {
    // Registrar el error pero no interrumpir
    error_log('QRHelper falló (no bloqueante): ' . $e->getMessage() . ' en ' . $e->getFile() . ' línea ' . $e->getLine());
}
// ---------------------------------
