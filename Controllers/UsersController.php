...
class UsersController
{
    ...
    public function store(Request $request, Response $response, mysqli $db): Response
    {
        ...
        $maxPhoneLength = 30;
        if (strlen($telefono) > $maxPhoneLength) {
            $_SESSION['error_message'] = "El número de teléfono no puede exceder los {$maxPhoneLength} caracteres";
    ...
    public function update(Request $request, Response $response, mysqli $db, int $id): Response
    {
        ...
        if (strlen($telefono) > $maxPhoneLength) {
            $_SESSION['error_message'] = "El número de teléfono no puede exceder los {$maxPhoneLength} caracteres";
            return $response->withHeader('Location', '/admin/utenti/modifica/' . $id . '?error=phone_too_long')->withStatus(302);
        }
    ...
    public function details(Request $request, Response $response, mysqli $db, int $id): Response
    {
        ...
        $sesiones = $auditModel->getUserSessions($id, 50);
    ...
    public function approveAndSendActivation(Request $request, Response $response, mysqli $db, array $args): Response
    {
        ...
        $tokenExpiry = gmdate('Y-m-d H:i:s', time() + (7 * 24 * 60 * 60));
    ...
    public function exportCsv(Request $request, Response $response, mysqli $db): Response
    {
        ...
        if (!empty($bindValues)) {
            ...
            if ($stmt === false) {
                $response->getBody()->write('Error preparing query');
            ...
            if ($result === false) {
                $response->getBody()->write('Error executing query');
                return $response->withStatus(500);
            }
        }
        ...
        $headers = [
            'ID', 'Codice Tessera', 'Nome', 'Cognome', 'Email', 'Telefono', 'Indirizzo',
            'Data Nascita', 'Sesso', 'Data Registrazione', 'Data Scadenza Tessera',
            'Stato', 'Tipo Utente', 'Codice Fiscale', 'Note Utente', 'Email Verificata',
            'Data Ultimo Accesso', 'Created At', 'Updated At'
        ];
