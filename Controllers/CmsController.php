...
class CmsController
{
    ...
    public function editHome(Request $request, Response $response, \mysqli $db, array $args): Response
    {
        ...
        // Ensure 'events' section exists in DB (self-healing)
        if (!isset($sections['events'])) {
            $stmt = $db->prepare("
                INSERT INTO home_content (section_key, title, subtitle, is_active, display_order)
                VALUES ('events', 'Eventi in Programma', 'Partecipa ai nostri prossimi incontri', 1, 9)
            ");
            $stmt->execute();
            $newId = $stmt->insert_id;
            $stmt->close();

            // Add to current sections array
            $sections['events'] = [
                'id' => $newId,
                'section_key' => 'events',
                'title' => 'Eventi in Programma',
                'subtitle' => 'Partecipa ai nostri prossimi incontri',
                'is_active' => 1,
                'display_order' => 9
            ];
        }
